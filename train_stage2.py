...
from methods import LabelAwareSmoothing, LearnableWeightScaling, FocalLoss
import pdb

# python train_stage2.py --cfg config/cifar10_trvaltt/cifar10_premixup0.2focal_stage2las.yaml  

def parse_args():
    parser = argparse.ArgumentParser(description='training (Stage-2)')
...
#                 best_acc1 = best_acc1.to(config.gpu)
    ...
    if config.use_focal:
        criterion = FocalLoss(gamma = 2, size_average = True)
    ...
    for epoch in range(config.num_epochs):
        ...
        test_acc1, test_ece = validate(test_loader, model, classifier, lws_model, criterion, config, logger, block)
