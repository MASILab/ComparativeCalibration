...
#                 best_acc1 = best_acc1.to(config.gpu)
    ...
    opt_T = validate(val_loader, model, classifier, lws_model, criterion, config, logger, phase = 'val')
    validate(test_loader, model, classifier, lws_model, criterion, config, logger, phase = 'test', opt_T = opt_T)
...
def validate(val_loader, model, classifier, lws_model, criterion, config, logger, phase, opt_T = None):
    ...
    with torch.no_grad():
        ...
        if phase == 'test':
            output_np = np.concatenate(output_all, axis = 0) / opt_T
            prob = softmax(output_np)
            
            confidence = prob.max(axis = 1)
            #
            np.save(config.log_dir.replace('logs', '') + 'test_confid_tscale.npy', confidence)
            np.save(config.log_dir.replace('logs', '') + 'test_pred_tscale.npy', pred_class)
            np.save(config.log_dir.replace('logs', '') + 'test_true_tscale.npy', true_class)
            cal = calibration(true_class, pred_class, confidence, num_bins=15)
            
            logger.info ('Testing Phase: opt_T, after_temperature_ece: ', opt_T, cal['expected_calibration_error'])
            ...
            for i in range(1, 101):
                ...
                output_np = np.concatenate(output_all, axis = 0) / T
                ...
                after_temperature_ece = cal['expected_calibration_error']

                if ece_val > after_temperature_ece:
                    T_opt_ece = T
                    #opt_ece = after_temperature_ece
                    ece_val = after_temperature_ece
            print ('T_opt_ece, opt_ece: ', T_opt_ece, ece_val)
