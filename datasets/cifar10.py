...
class IMBALANCECIFAR10(torchvision.datasets.CIFAR10):
    ...
    def update_idx(self, idx):
        ...
        for i in range(len(idx)):
            mask.append(idx[i] in TRAIN_idx)
        return mask #idx[np.array(mask)]
    ...
    def gen_imbalanced_data(self, img_num_per_cls):
        ...
        for the_class, the_img_num in zip(classes, img_num_per_cls):
            ...
            #print (len(idx), the_class, the_img_num)
            mask = self.update_idx(idx)
            idx = idx[np.array(mask)]

            #np.random.shuffle(idx)
            
            selec_idx = idx[:the_img_num]
            print (len(selec_idx), the_class, the_img_num)
...
class CIFAR10_LT(object):
    ...
    def __init__(self, distributed, root='/nfs/masi/gaor2/data/cifar10', imb_type='exp', imb_factor=0.01, batch_size=128, num_works=40):
        ...
        if imb_factor > 1.0:
            ...
            print ('using the normal loader, not imb ---------------')
            self.cls_num_list = [5000] * 10
        ...
        eval_dataset = torchvision.datasets.CIFAR10(root=root, train=True, download=False, transform=eval_transform)
...
#         train_dataset = IMBALANCECIFAR10(root=root, imb_type=imb_type, imb_factor=imb_factor, rand_number=0, train=True, download=True, transform=train_transform)
        ...
        self.train_instance = torch.utils.data.DataLoader(
            ...
            num_workers=num_works, pin_memory=True, sampler=train_sampler)
        ...
        self.eval = torch.utils.data.DataLoader(
            eval_dataset,
            batch_size=batch_size, shuffle=False,
            num_workers=num_works, sampler = valid_sampler, pin_memory=True)
