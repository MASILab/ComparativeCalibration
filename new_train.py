...
from methods import mixup_data, mixup_criterion, FocalLoss
import pdb
...
def main():
    ...
    if not os.path.exists(config.code_dir):
        os.mkdir(config.code_dir)
    
    files = os.listdir('./')
    for file in files:
        if file[0] != '.':
            os.system('cp -r ' + file + ' ' + config.code_dir)
            os.system('cp ' + args.cfg + ' ' + config.code_dir)
...
#     start_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    ...
    if config.largenet == False and (config.dataset == 'cifar10' or config.dataset == 'cifar100'):
        model = getattr(resnet_cifar, config.backbone)() # 64
        classifier = getattr(resnet_cifar, config.classifier)(64, config.num_classes)
    
    

    elif config.dataset == 'imagenet' or config.dataset == 'ina2018' or (config.dataset in ['cifar10', 'cifar100'] and config.largenet == True):
        model = getattr(resnet, config.backbone)()
        classifier = getattr(resnet_cifar, config.classifier)(2048, config.num_classes)
    ...
    if config.use_focal: 
        criterion = FocalLoss(gamma = 2, size_average = True).cuda(config.gpu)
    else:
    ...
    for epoch in range(config.num_epochs):
       ...
       # test_acc1, test_ece = validate(test_loader, model, classifier, criterion, config, logger, block)
        ...
        if is_best:
            its_ece = ece
            best_epoch = epoch
        logger.info('Best Epoch: %d Best Prec@1: %.3f%% ECE: %.3f%%\n' % (best_epoch, best_acc1, its_ece))
   ...
   # top5 = AverageMeter('Acc@5', ':6.3f')
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if config.mixup is True:
            ...
            feat = model(images)
            if config.classifier == 'Classifier':
                output = classifier(feat)
            else:
                output = classifier(feat, target)
